...
<style>
    ...
    .current-image img { width:100px; height:100px; object-fit:cover; border-radius:8px; border:2px solid #ddd; }
...
<div class="page-container">
    ...
    <div class="card">
        <form action="{{ route('applink.update', $app->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <div class="form-group">
                <label for="name">Nama Aplikasi</label>
                <input type="text" id="name" name="name" value="{{ old('name', $app->name) }}" required>
                @error('name')
            ...
            <div class="form-group">
                ...
                <input type="url" id="url" name="url" value="{{ old('url', $app->url) }}" placeholder="https://contoh.com" required>
                @error('url')
                    <div class="error">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                @if($app->image)
                    <div class="current-image">
                        <p style="font-size:12px; color:#666; margin-bottom:8px;">Logo saat ini:</p>
                        <img src="{{ asset('storage/'.$app->image) }}" alt="{{ $app->name }}">
                    </div>
                @endif
                <input type="file" id="image" name="image" accept="image/*">
                <div class="help-text">Kosongkan jika tidak ingin mengubah logo</div>
                @error('image')
                    <div class="error">{{ $message }}</div>
                @enderror
            </div>
